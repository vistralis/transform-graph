...
def _get_heuristic_root(transform_graph: TransformGraph) -> str:
    ...
    candidate_lists = [
        ["world", "map", "global", "city", "origin"],
        ["base_link", "base", "root", "ego", "egovehicle", "vehicle", "car"],
        ["lidar", "lidar_top", "os_sensor"],
        ["odom"],
    ]
    candidates = [item for sublist in candidate_lists for item in sublist]
...
def visualize_transforms(
    ...
    ignore_frames: list[str] | None = None,
    ...
    Args:
        transform_graph: The TransformGraph to visualize.
        target_frame: The frame to use as the target (origin) of the visualization.
                   If None, heuristically selects a root frame.
        axis_scale: Scale factor for the basis vector axes.
        show_connections: Whether to draw lines connecting frames.
        show_frustums: Whether to show camera frustums.
        frustum_scale: Scale factor for camera frustums.
        ignore_frames: List of frame IDs to ignore (e.g., ['global']).
    ...
    if target_frame is None:
        target_frame = _get_heuristic_root(transform_graph)

    if not transform_graph.has_frame(target_frame):
        raise ValueError(f"Target frame '{target_frame}' not found in graph.")

    traces = []
    frame_transforms = {}
    frames = transform_graph.frames
    if ignore_frames:
        frames = [f for f in frames if f not in ignore_frames]
    ...
    for u, v, edge_data in transform_graph.graph.edges(data=True):
        ...
        if isinstance(tr, (Projection, InverseProjection)):
            target_frame_edge = edge_data.get("reference_frame")
            other_frame = v if target_frame_edge == u else u
            ...
            if isinstance(tr, InverseProjection) and not isinstance(tr, CameraProjection):
                ...
                # Edge: target_frame_edge=Target(3D), other=Source(2D)
                target_2d = other_frame
                source_3d = target_frame_edge
            else:
                # Projection/CameraProjection (Source=3D, Target=2D)
                # Edge: target_frame_edge=Target(2D), other=Source(3D)
                target_2d = target_frame_edge
                source_3d = other_frame

            if target_2d and source_3d:
                projection_map_2d_to_3d[target_2d] = source_3d

    # Compute global transforms relative to target
    for frame_id in frames:
        try:
            if frame_id == target_frame:
                ...
                transform = transform_graph.get_transform(target_frame_for_transform, target_frame)
    ...
    for frame_id, transform in frame_transforms.items():
        traces.extend(create_axis_traces(transform, scale=axis_scale, name=frame_id))
        ...
        if show_frustums:
            ...
            for u, v, edge_data in transform_graph.graph.edges(data=True):
                ...
                # Case 3: Edge holds CompositeProjection
                # Source=Camera, Target=Image.
                # reference_frame=Image, other=Camera.
                elif isinstance(edge_transform, CompositeProjection):
                    if other_frame == frame_id and isinstance(
                        edge_transform.projection, CameraProjection
                    ):
                        intrinsic_to_draw = edge_transform.projection

                # Case 4: Edge holds InverseCompositeProjection
                # Source=Image, Target=Camera.
                # reference_frame=Camera, other=Image.
                elif isinstance(edge_transform, InverseCompositeProjection):
                    if reference_frame == frame_id and isinstance(
                        edge_transform.projection.inverse(), CameraProjection
                    ):
                        intrinsic_to_draw = edge_transform.projection.inverse()
                ...
                if intrinsic_to_draw is not None:
                    ...
                    if hasattr(pose, "rotation") and hasattr(pose, "translation"):
                        # Already a good Transform
                        pass
                    elif hasattr(pose, "as_matrix"):
                        # Try to convert MatrixTransform to Transform to get rotation/translation
                        try:
                            pose = Transform.from_matrix(pose.as_matrix())
                        except Exception:
                            # If extraction fails (e.g. non-rigid), skip specific frustum
                            continue
                    ...
                    if hasattr(pose, "rotation") and hasattr(pose, "translation"):
                        ...
                        # Don't break, allowing multiple frustums if multiple cameras attached
                        # (though unlikely for one frame)
...
visualize_transform_graph = visualize_transforms
visualize_graph = inspect_graph
