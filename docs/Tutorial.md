...
rotation = tf.Rotation.from_euler_angles(yaw=np.pi/2)
...
pose = tf.Pose(
    ...
    orientation=tf.Rotation.from_euler_angles(yaw=0.5).rotation,
    frame_id='map',
...
graph.add_transform('base', 'arm', tf.Transform(
    ...
    rotation=tf.Rotation.from_euler_angles(pitch=0.5).rotation
))
